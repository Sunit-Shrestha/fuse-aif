...
POST /research        -- W16 agentic loop: model-driven multi-step search + verification.
...
from .schemas import ChatRequest, ChatResponse, ResearchRequest, ResearchResponse
...
async def lifespan(app: FastAPI):
    ...
    _state["agent"] = ResearchAgent(rag_index)
    yield
    _state.clear()
...
def research(req: ResearchRequest):
    return _state["agent"].run(req.message)
